...
/*
* TAT : Turn Around Time = CT - AT(arrival time)
* WT : Waiting Time = TAT - BT(Burst Time)
* RT : Response Time = WT
* Throughput = no. of total processes / Total Completion time of all Processes
* CPU Utilization (%) = ( Total CPU Utilization / Total Completion time of all Processes ) * 100
*/


// ★ pid를 들어온 순서대로 큐로 묶어서! (구조체 배열)
//	  그리고 at, bt를 배열로 해서 구조체로 묶기?
struct process_struct {
	int pid;
	int at; // arrival time
	int bt; // cpu_burst time (CPU)
...
int main() {
	...
	// Calcuate Length of Process completion Cycle
	...
	printf("\nCPU Utilization(Percentage)= %f ", cpu_utilization);
